...
test("server-renders the Juliana content hub for a selected region", async () => {
  ...
  assert.match(html, /Veterinária, agropecuarista, casada e mãe/);
  assert.match(html, /Agronegócio forte/);
  assert.match(html, /Juliana é 1020/);
  ...
  assert.match(html, /Juliana — Forró de vaquejada/);
  assert.match(html, /Saúde em Barra do Garças/);
